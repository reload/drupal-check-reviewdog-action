...
/usr/bin/drupal-check --memory-limit=-1 "--${INPUT_CHECK}" --format=checkstyle --no-progress --no-interaction -- "${PATHS[@]}" | tee /dev/stderr | /usr/bin/reviewdog -f=checkstyle -name="Analysis" -reporter="${INPUT_REPORTER}"
